...
/* ==========================================================================
   10. SYN_DEBUG proving ground — SCAFFOLDING, DELETE AT STAGE 6c

   Stage 6a step 1 builds the engine and is forbidden from building a template
   (stage-6-handoff-prompt.md §9), so there is no templates/service.php for a
   field box to attach to yet. Without something to attach to there is nothing to
   test, and testing the repeater by hand is the whole point of the stop after
   this step.

   So: one group, exercising all four field types and all six leaf types,
   attached to the default page template, and registered ONLY when SYN_DEBUG is
   true — which is staging and local, never production (functions.php). Its meta
   keys all start _syn_demo_, so they are trivial to find and delete.

   Delete this whole block, and nothing else, when templates/service.php lands.
   ========================================================================== */

if ( SYN_DEBUG ) {

	add_action( 'syn_register_fields', 'syn_register_demo_field_group' );
	/**
	 * Registers the proving-ground field group.
	 *
	 * Side effects: registers one field group on the default page template.
	 *
	 * @return void
	 */
	function syn_register_demo_field_group() {
		syn_register_field_group(
			array(
				'id'          => 'demo',
				'title'       => __( 'Field engine — test bench (staging only)', 'synergi' ),
				'description' => __( 'Scaffolding for Stage 6a. It appears on pages using the Default template, only while SYN_DEBUG is on, and is deleted when the service template lands. Switching a page template does not move this box until the page has been saved and reloaded.', 'synergi' ),
				'post_types'  => array( 'page' ),
				...
				'fields'      => array(
					array(
						'key'         => 'demo_eyebrow',
						'type'        => 'text',
						'label'       => __( 'Eyebrow (simple text)', 'synergi' ),
						'description' => __( 'A short label above a heading.', 'synergi' ),
						'default'     => __( 'What we do', 'synergi' ),
						'max_length'  => 80,
					),
					array(
						'key'         => 'demo_lede',
						'type'        => 'textarea',
						'label'       => __( 'Lede (multi-line text)', 'synergi' ),
						'description' => __( 'One or two sentences. Line breaks are kept, markup is not.', 'synergi' ),
						'default'     => __( 'A boutique business process outsourcing partner, built in the Gulf.', 'synergi' ),
						'rows'        => 3,
					),
					array(
						'key'           => 'demo_image',
						'type'          => 'image',
						'label'         => __( 'Photograph (media picker)', 'synergi' ),
						'description'   => __( 'Chosen from the media library. The theme stores the picture, not a file path.', 'synergi' ),
						'fallback_slug' => 'synergi-hero',
					),
					array(
						'key'     => 'demo_cta',
						'type'    => 'link',
						'label'   => __( 'Call to action (link)', 'synergi' ),
						'default' => array(
							'url'   => '',
							'label' => __( 'Talk to us', 'synergi' ),
						),
					),
					array(
						'key'       => 'demo_rows',
						'type'      => 'repeater',
						'label'     => __( 'Capabilities (repeater)', 'synergi' ),
						...
						'subfields' => array(
							...
							array(
								'key'   => 'summary',
								'type'  => 'textarea',
								'label' => __( 'Summary (plain text)', 'synergi' ),
								'rows'  => 3,
							),
							array(
								'key'         => 'answer',
								'type'        => 'html',
								'label'       => __( 'Answer (allows basic HTML)', 'synergi' ),
								'description' => __( 'The leaf type the FAQ answers will use in Stage 6c.', 'synergi' ),
								'rows'        => 4,
							),
							array(
								'key'   => 'url',
